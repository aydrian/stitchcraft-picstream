exports = function(changeEvent) {
  ...
  const picstream = mongodb.db('data').collection('picstream');
  
  const { fullDocument } = changeEvent;
  const { user_id, full_name } = fullDocument;
  
  return picstream.updateMany({owner_id: user_id }, {"$set": {owner_name: full_name}});
  /*
